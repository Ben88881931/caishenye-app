...
- “下期预估”必须使用 `prediction_snapshots.json` 的开奖前加权反弹快照；历史对错、上期反馈和当前推荐都读取 `snapshots.js` 的 `weightedRecords`，不得在遇到并列时把真实推荐改显示为“跳过”。历史记录从第1期起点开始，第2期起展示回测，262期起用真实快照覆盖对应期数；首推和备选像双号推荐一样各自独立记录，只统计各自的最高连错、当前连错和逐期命中/未中，不增加组合统计。历史业绩必须与双号推荐一致，计算单号命中率、至少中1个、累计盈亏、最大回撤和中0/中1/中2。
...
- 导航自定义排序：导航末尾保留「导航排序」按钮，点击进入排序模式（每个标签旁显示 ↑/↓ 箭头），再点退出；排序模式有`退出排序`与`恢复默认`两个控件。
- 导航排序调整后立即保存到 localStorage `v2_tab_order`（只存标签 id 数组）；「恢复默认」清除保存并恢复 TABS 原始顺序；保存内容损坏或非数组时自动恢复默认。
- 导航顺序合并规则：已保存且仍存在的标签按保存顺序排，新增标签自动追加末尾，已删除标签自动忽略；不得直接修改 `TABS` 原始定义，`renderTabs()` 只能通过 `getVisibleTabs()` 生成导航。
- 排序模式切换只重渲染导航，不得触发页面内容重渲染；当前正在查看的页面在排序后必须保持选中。
- 排序按钮与箭头按钮触控区域不小于 36px；手机端横向滑动、桌面端自动换行；文字与箭头不得重叠。
