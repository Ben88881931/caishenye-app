...
- “下期预估”必须使用 `prediction_snapshots.json` 的开奖前加权反弹快照；历史对错、上期反馈和当前推荐都读取 `snapshots.js` 的 `weightedRecords`，不得在遇到并列时把真实推荐改显示为“跳过”。历史记录从第1期起点开始，第2期起展示回测，262期起用真实快照覆盖对应期数；首推和备选像双号推荐一样各自独立记录，只统计各自的最高连错、当前连错和逐期命中/未中，不增加组合统计。
