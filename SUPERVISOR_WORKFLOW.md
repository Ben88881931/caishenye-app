...
页面改版必须保持“先结论、后明细”，并满足以下要求：

- 顶部导航按钮高度不得小于 42px，字号不得小于 14px，手机端必须易于点击。
- 宽表必须支持横向滑动；第一列需要固定，避免滑动后看不清尾号。
- “尾号性格”先显示当下速览，再显示遗漏后反弹、连出后延续明细。
- “尾号性格”的遗漏和连出明细统一展示 1 至 10 期，不得只显示前 3 期。
- “遗漏监控”展示最近 15 次已完成的历史遗漏，当前遗漏继续单独显示。
- “窗口走势”必须展示窗口范围、当前遗漏/连出、开出生数、命中率、理论偏差、升温/降温、最新状态和逐期结果。
- “生肖窗口走势”必须支持 5/7/10/15/21/30 期窗口、年份切换、当前遗漏/连出、实际出现率、按生肖号码个数计算的理论偏差、升温/降温和逐期结果。
