#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""从仓库根目录的老版数据生成 v2/data.js（供 GitHub Actions 自动同步）。"""
...
def main():
    raw = json.load(open(os.path.join(ROOT, "lottery_data.json"), encoding="utf-8"))
    raw = {k: v for k, v in raw.items() if k.isdigit()}
    raw = {k: raw[k] for k in sorted(raw, key=int)}

    html = open(os.path.join(ROOT, "号码走势图.html"), encoding="utf-8").read()
